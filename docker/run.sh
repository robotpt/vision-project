...
case "$MODE" in
	run | start | up )
		ROS_MASTER_URI=$ROS_MASTER_URI docker-compose run $SERVICE_NAME bash -c "export PYTHONPATH='${PYTHONPATH}:/root/catkin_ws/src/mongodb-statedb' && source ~/catkin_ws/devel/setup.bash && roslaunch ros_vision_interaction vision_project.launch"
		;;
